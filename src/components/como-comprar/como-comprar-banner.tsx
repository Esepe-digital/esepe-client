...
interface ComoComprarBannerProps {
  titulo?: string;
  texto?: string;
  imagen?: string;
}

export default function ComoComprarBanner({
  titulo = 'Proceso de compra',
  texto = 'El proceso de compra de tu hogar es una de las decisiones más importantes que tomarás y en ESEPE queremos ayudarte a que sea lo más fácil y claro posible. Por ello nuestro ejecutivo te acompañará durante todo el proceso.',
  imagen,
}: ComoComprarBannerProps) {
  const handleScroll = () => {
    window.scrollTo({
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:flex-1 h-[450px] sm:h-[350px] lg:h-auto relative overflow-hidden">
        <Image
          src={imagen || '/default-banner.jpg'}
          alt="Proceso de Compra Banner"
          fill
